...
const currentVersions={homeCss:20,siteCss:7,zhCss:25,homeJs:23,siteJs:17};
...
function head(title,description,file) {
  ...
  return `<!DOCTYPE html>\n<html lang="zh-CN">\n<head>\n<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="author" content="杨杰, Jie Yang, 西湖大学"><title>${esc(title)} — 杨杰 | 西湖大学</title><meta name="description" content="${esc(description)}"><link rel="icon" href="static/assets/favicon.ico"><link rel="preload" href="static/assets/fonts/inter/inter-latin-wght-normal.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="static/css/en-home.css?v=${currentVersions.homeCss}"><link rel="stylesheet" href="static/css/en-site.css?v=${file==='join.html'?9:file==='coverage.html'?11:currentVersions.siteCss}"><link rel="stylesheet" href="static/css/zh-locale.css?v=${file==='index.html'?41:file==='research.html'?31:file==='publications.html'?39:currentVersions.zhCss}"><link rel="alternate" hreflang="en" href="${route(file)}"><link rel="alternate" hreflang="zh-CN" href="${file}"><script type="application/ld+json">${JSON.stringify(schema).replace(/</g, '\\u003c')}</script>\n</head>`;
}
